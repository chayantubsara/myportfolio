...
export interface AwardRecord extends BaseRecord { name:string; issuer:string; issuedDate:string; description:string; documentId:string; publicDocument:boolean; featured:boolean }
...
export interface PortfolioData { profile:Profile|null; education:Education[]; experience:Experience[]; projects:Project[]; certifications:Certification[]; awards:AwardRecord[]; skills:Skill[]; socialLinks:SocialLink[]; documents:DocumentRecord[]; languages:{name:string;level:string}[]; lastUpdated:string }
